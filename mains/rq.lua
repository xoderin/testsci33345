...
-- Clone Template for Dropdown
local DropdownTemplate = OriginFrame:Clone()
DropdownTemplate.ItemName.Text = "Select Song"
DropdownTemplate.ItemIcon.Image = "rbxassetid://12072054746" -- Use a different icon if needed
DropdownTemplate.Visible = true
DropdownTemplate.Parent = Items

-- Create a ScrollingFrame to hold the Dropdown options
local DropdownOptions = Instance.new("ScrollingFrame")
DropdownOptions.Size = UDim2.new(0, 250, 0, 300) -- Width: 250px, Height: 300px
DropdownOptions.Position = UDim2.new(1, 10, 0, 0) -- Position to the right of the button
DropdownOptions.BackgroundTransparency = 0
DropdownOptions.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
DropdownOptions.BorderSizePixel = 0
DropdownOptions.ScrollBarThickness = 8
DropdownOptions.ScrollBarImageColor3 = Color3.fromRGB(100, 100, 100)
DropdownOptions.CanvasSize = UDim2.new(0, 0, 0, 0) -- Adjust dynamically
DropdownOptions.AutomaticCanvasSize = Enum.AutomaticSize.Y
DropdownOptions.ZIndex = 10
DropdownOptions.Parent = DropdownTemplate
...
-- Add a UIPadding to add spacing around the edges
...
-- Populate the Dropdown with Audio IDs
local audioList = {
    {7067700233, "yoaimo"},
    {8501619302, "female of above"},
    {7431081635, "gambare gambare =))"},
    {7426001582, "lol loud girl anime"},
    {7896694622, "gojo music"},
    {5995732279, "muma loud"},
    {5455280246, "tran duc bo hoc tieng meo keu"},
    {7311729680, "loud Chinese rap"},
    {7311729680, "dong thap flute loud"},
    {6557459238, "bomman"},
    {8842446965, "nya argiato"},
    {7426001582, "senpai music"},
    {18841888868, "d3r - love bomb phonk"},
    {17532729202, "kobeni"},
    {110789178293428, "brainrot TikTok music"},
    {18805592597, "yes king"},
    {16662827543, "pluh music"},
    {17647322226, "kerosense loud"},
    {17866420285, "kurwa"},
    {95920078579068, "Russian save Europe lol"},
    {18841888491, "Rizzlers and Moggers"},
    {892233254, "scream"},
    {1129332284, "tuturu"},
    {1076907875, "discord ping"}
}

for _, audio in ipairs(audioList) do
    local audioButton = Instance.new("TextButton")
    audioButton.Size = UDim2.new(1, 0, 0, 30)
    audioButton.Text = audio[2] .. " (" .. audio[1] .. ")"
    audioButton.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
    audioButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    audioButton.Font = Enum.Font.Gotham
    audioButton.TextScaled = false
    audioButton.TextSize = 14
    audioButton.AutoButtonColor = false
    audioButton.ZIndex = 11 -- Ensure buttons are above the ScrollingFrame
    audioButton.Parent = DropdownOptions -- Properly parent the button to the ScrollingFrame
    ...
    UIStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    UIStroke.Parent = audioButton

    -- Add rounded corners
    local UICorner = Instance.new("UICorner")
    UICorner.CornerRadius = UDim.new(0, 4)
    UICorner.Parent = audioButton

    -- Handle button click
    audioButton.MouseButton1Click:Connect(function()
        TextBox.Text = tostring(audio[1])
        TextBox2.Text = "10000" -- Default volume set to 100
        Notify("Selected: " .. audio[2], 2, 0.5)
    end)
end

-- Add a UICorner to the ScrollingFrame for rounded edges
local UICorner = Instance.new("UICorner")
UICorner.CornerRadius = UDim.new(0, 8)
UICorner.Parent = DropdownOptions
...
--S(ID,instance,Volume:number,Looped:boolean,LocalVolume:number)
function QS(ID)
...
for i = 1,250 do
	task.wait(0.05)
	task.spawn(function()
	S(892233254,workspace,10,true,0.02)
end)
	task.wait()
end
end)
